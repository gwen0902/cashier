...
if(isset ($_SESSION['username'])){
  $query_user = getData("select * from tb_user natural join tb_level where id_user = $id");
  foreach($query_user as $r){
...
<!--jika sudah order-->
      ...
      <?php
        $order = array();
        $query_lihat_order = getData("select * from tb_order");
        foreach($query_lihat_order as $r_dt_order ){
    ...
    <div class="row-fluid">
      <div class="span8">
        <div class="widget-box span12">
          <div class="widget-title bg_lg"><span class="icon"><i class="icon-th-large"></i></span>
            ...
            <form action="entri_order.php" method="get">
              <div id="search">
                  <input type="text" name="keyword" placeholder="Cari Menu..."/>
                  <button type="submit" name="cari"  class="tip-bottom" title="Search"><i class="icon-search icon-white" ></i></button>
              </div>
            </form>
            ...
            <ul class="thumbnails">
              <div class="btn-icon-pg">
                <ul>
                  ...
                  <?php 
                    $pesan = array();
                    $query_lihat_pesan = getData("select * from tb_pesan where id_user = $id and status_pesan != 'sudah'");
                    foreach($query_lihat_pesan as $r_dt_pesan){
                      array_push($pesan, $r_dt_pesan['id_masakan']);
                    }
                      $query_data_makanan = getData("select * from tb_masakan where stok > 0 order by id_masakan desc");
                      $no_makanan = 1;
                      if(isset($_GET['cari'])){
                        $query_data_makanan = cari($_GET['keyword']);
                      }
                    foreach($query_data_makanan as $r_dt_makanan){
                  ?>
                      ...
                      <li class="span2"> 
                        ...
                        <table class="table table-bordered">
                          <tbody>
                            <tr>
                              <td><strong><?php echo $r_dt_makanan['nama_masakan'];?></strong></td>
                            </tr>
                            <tr>
                              <td><?php echo $r_dt_makanan['detail_menu'];?></td>
                            </tr>
                            <tr>
                              <td>Harga / Porsi</td>
        ...
        <div class="widget-box">
          ...
          <div class="widget-content nopadding">
            <table class="table table-bordered table-striped">
              ...
              <tbody>
              <?php
                $query_draft_pesan = getData("select * from tb_pesan natural join tb_masakan where id_user = $id and status_pesan != 'sudah'");
                foreach($query_draft_pesan as $r_draft_pesan){
              ?>
                <tr class="odd gradeX">
      ...
      <?php
          ...
          if(isset($_POST['hapus_pesan'])){
            $hapus_pesan = hapus_pesan();
          }

          if(isset($_POST['proses_pesan'])){
           $proses_pesan = proses_pesan();
          }
        }
      ?>
    </div>
